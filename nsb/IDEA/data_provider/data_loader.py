...
class Dataset_ECG500(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        data = pd.read_csv(os.path.join(root_path, data_path), header=None)
        data = data.sample(frac=1, random_state=2023)
        length = data.shape[0]
        lengths = [0, int(length * 0.6), int(length * 0.8), length]
        data = data.values
        if self.scale:
            self.mean = np.mean(data[lengths[0]:lengths[1]])
            self.std = np.std(data[lengths[0]:lengths[1]])
            self.data = (data[lengths[self.set_type]:lengths[self.set_type + 1]] - self.mean) / self.std
        else:
            self.data = data[lengths[self.set_type]:lengths[self.set_type + 1]]
        # ECG
        self.data = np.expand_dims(self.data, axis=-1)

    def __getitem__(self, index):
        seq_x = self.data[index, :self.seq_len]
        seq_y = self.data[index, self.seq_len - self.label_len:self.seq_len + self.label_len + self.pred_len]
...
class Dataset_SYN_Data(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=False, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
