...
class Exp_NSTS_Pre(Exp_Basic):
    def __init__(self, args):
        super(Exp_NSTS_Pre, self).__init__(args)
    ...
    def train(self):
        ...
        dataset = TensorDataset(
            ...
            torch.cat(batch_x_mark_list, dim=0), torch.cat(batch_y_mark_list, dim=0))
        train_data = dataset
        ...
        for epoch in range(self.args.pre_epoches, self.args.train_epochs):
            ...
            for i, (batch_u, batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    train_loss.append(loss.item())
                batch_x_list.append(torch.cat((batch_x, batch_y), dim=1).cpu())
                batch_y_list.append(batch_y.cpu())
                batch_x_mark_list.append(batch_x_mark.cpu())
                batch_y_mark_list.append(batch_y_mark.cpu())
                batch_u_list.append(U.detach().cpu())
            ...
            # os.makedirs(f"./draw_data/{self.args.model}/{self.args.data}/{self.args.pred_len}/", exist_ok=True)
            # np.save(f'./draw_data/{self.args.model}/{self.args.data}/{self.args.pred_len}/u.npy',
            #         np.array(batch_u_list))
            # np.save(f'./draw_data/{self.args.model}/{self.args.data}/{self.args.pred_len}/true.npy',
            #         np.array(batch_x_list))
            train_loss = np.average(train_loss)
            vali_loss = self.vali(vali_data, vali_loader, criterion, epoch)
            test_loss = self.vali(test_data, test_loader, criterion, epoch)
    ...
    def test(self):
        ...
        f.write(
            f"zc_{self.args.zc_kl_weight}_zd_{self.args.zd_kl_weight}_hmm_{self.args.hmm_weight}_pre_epoches_{self.args.pre_epoches}\n  ")
