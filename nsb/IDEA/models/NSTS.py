...
class Base_Net(nn.Module):
    def __init__(self, input_len, out_len, input_dim, out_dim, is_mean_std=True, activation="gelu",
        ...
        if self.c_type == "None":
            self.net = MLP1(layer_nums, in_dim=input_len, out_dim=out_len * self.radio, hid_dim=out_len * 2,
                            activation=activation,
                            layer_norm=layer_norm)
        elif self.c_type == "type1":
            self.net = MLP1(layer_nums, in_dim=self.input_dim, hid_dim=self.out_dim * 2,
                            out_dim=self.out_dim * self.radio,
                            layer_norm=layer_norm, activation=activation)
        elif self.c_type == "type2":
            self.net = MLP1(layer_nums, in_dim=self.input_dim * input_len, hid_dim=self.out_dim * 2 * input_len,
                            activation=activation,
...
class Encoder_ZC(nn.Module):
    def __init__(self, configs) -> None:
        ...
        if configs.enc_in < 100:
            ...
            self.stationary_transition_prior = NPTransitionPrior(lags=self.configs.lags,
                                                                 ...
                                                                 num_layers=1,
                                                                 hidden_dim=3)
        self.zc_rec = Base_Net(input_len=configs.seq_len, out_len=configs.seq_len, input_dim=configs.enc_in,
                               out_dim=configs.zc_dim, is_mean_std=True, drop_out=configs.dropout,
                               layer_nums=configs.hidden_layers, layer_norm=configs.is_bn)

        self.zc_pred = Base_Net(input_len=configs.seq_len, out_len=configs.pred_len, input_dim=configs.zc_dim,
                                out_dim=configs.zc_dim, is_mean_std=True, drop_out=configs.dropout,
                                layer_nums=configs.hidden_layers, layer_norm=configs.is_bn)
    ...
    def forward(self, x_enc, is_train=True):
        ...
        zc_rec_mean, zc_rec_std = self.zc_rec(x_enc)
        # print("zc_rec_mean:shape", zc_rec_mean.shape)
        # print("zc_rec_std:shape", zc_rec_std.shape)
        ...
        zc_pred_mean, zc_pred_std = self.zc_pred(zc_rec_mean)
        # print("zc_pred_mean:shape", zc_pred_mean.shape)
        # print("zc_pred_std:shape", zc_pred_std.shape)
        ...
        return (zc_rec_mean.permute(0, 2, 1), zc_rec_std.permute(0, 2, 1), zc_rec.permute(0, 2, 1)), (
            zc_pred_mean.permute(0, 2, 1), zc_pred_std.permute(0, 2, 1), zc_pred.permute(0, 2, 1))
...
class Decoder(nn.Module):
    def __init__(self, configs) -> None:
        ...
        self.configs = configs
        self.z_net = nn.Linear(self.configs.zd_dim, self.configs.enc_in, bias=False)
        ...
        weight = torch.eye(configs.enc_in, self.configs.zd_dim)

        self.z_net.weight = nn.Parameter(weight)

    def forward(self, zc_rec, zd_rec, zc_pred, zd_pred):
        # print("zc_rec:shape", zc_rec.shape)
        # print("zd_rec:shape", zd_rec.shape)
        # print("zc_pred:shape", zc_pred.shape)
        # print("zd_pred:shape", zd_pred.shape)
        z_rec = self.z_net(zd_rec.permute(0, 2, 1)).permute(0, 2, 1)
        z_pred = self.z_net(zd_pred.permute(0, 2, 1)).permute(0, 2, 1)
...
class NPChangeTransitionPrior(nn.Module):
    ...
    def __init__(
        ...
        self.latent_size = latent_size
        self.lags = lags
        self.gs = nn.ModuleList([MLP2(input_dim=embedding_dim + 1 + latent_size, hidden_dim=hidden_dim,
                                      output_dim=1, num_layers=num_layers) for _ in range(latent_size)])
    ...
    def forward(self, x, embeddings):
        ...
        for i in range(self.latent_size):
            ...
            batch_inputs = torch.cat(
                (batch_embeddings, batch_x_lags[:, -1,:], batch_x_t[:, :, i]), dim=-1)
            # 并行不了
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, y_enc=None, is_train=True, is_out_u=False, c_est=None):
        ...
        if is_train and (not self.configs.No_prior):
            ...
            other_loss = zd_kl_loss * self.configs.zd_kl_weight + hmm_loss * self.configs.hmm_weight + other_loss
            if is_out_u:
