    ...
    for model in models_list:
        for pred_len in pred_len_list:
             for seed in seed_list:
                    seq_len = 96
                    label_len = 0
                    command = f"""     {c[data][str(pred_len)].replace("run.py", "tune.py")}  --model {model} --seq_len {seq_len}  --is_bn --label_len 0  --pred_len {pred_len} --devices 0,1,2,3,4,5,6,7 --use_multi_gpu   --train_epochs 40  --patience 5 --seed {seed} """
